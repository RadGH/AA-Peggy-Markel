...
class taxonomy_list_widget_plugin {
	 ...
	 * To ensure the widget continues to _includes/exclude the updated terms, we search widget options on terms split and update stored IDs.
	 *
...
class taxonomy_list_widget extends WP_Widget {
	...
	?>
		...
		<h3><?php _e( '_includes/Exclude Terms' ); ?></h3>

		<p>
			<label><?php _e( '_includes/exclude terms:' ); ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'incexc_ids' ); ?>"><?php _e( 'Term IDs to _includes/exclude based on above setting:' ); ?></label><br />
